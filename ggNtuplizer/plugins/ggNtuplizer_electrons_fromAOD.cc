...
void ggNtuplizer::fillElectrons(const edm::Event &iEvent, const edm::EventSetup &iSetup, math::XYZPoint &pv) {
  ...
  */
  ...
  iEvent.getByToken(recoelectronCollection_, electronHandle);
  ...
  for (edm::View<reco::GsfElectron>::const_iterator iEle = electronHandle->begin(); iEle != electronHandle->end(); ++iEle) {
    ...
    //eleSIP_             .push_back(fabs(iEle->dB(pat::Electron::PV3D))/iEle->edB(pat::Electron::PV3D)); // not present in reco::GsfElectron
